...
void main() {
  testWidgets('Calculator smoke test (renders)', (WidgetTester tester) async {
    ...
    // Tap a '+' button (icon may differ). Prefer keypad button by text fallback.
    final plusFinder = find.byIcon(Icons.add);
    if (plusFinder.evaluate().isNotEmpty) {
      await tester.tap(plusFinder);
      await tester.pump();
    } else {
      final plusTextFinder = find.text('+');
      if (plusTextFinder.evaluate().isEmpty) {
        // If '+' isn't available, just ensure the widget tree is present.
        return;
      }
      await tester.tap(plusTextFinder);
      await tester.pump();
    }
